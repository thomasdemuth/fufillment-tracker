...
every push. The site itself holds **no data**; it can show three things:

| Source | What it is | Where the data lives |
|---|---|---|
| **Demo** | Made-up shipments bundled with the site, opened by default on desktop | In the site's files (fake, generated by `make demo-snapshot`) |
| **Your own data, in this browser** | Upload spreadsheets right on the site: column mapping, board, map, attention, notes, tags, export, mock refresh | Your browser's storage (IndexedDB) on that device only |
| **Snapshot file / your server** | A file from "Send to phone", or a live connection to a server you run | The file, or your machine |

To use your own data, open the site and click **Use my own data** in the demo banner (or **Keep my data in this
browser** on the phone start screen). The demo disappears, the upload wizard opens, and everything you import is kept
only in that browser: it is never sent to GitHub, to a server, or anywhere else. The site is plain static files, so it
...
Keeping real data out of the repository: `.gitignore` refuses `*.snapshot.json` and spreadsheets anywhere in the tree
(except the fake `demo/` files and test fixtures), and the bundled demo is only ever regenerated by
`make demo-snapshot`, which seeds a throwaway database rather than reading yours.
...
make demo-snapshot   # regenerate the hosted site's bundled demo from the fake ./demo files (throwaway DB, never yours)
...
pnpm exec playwright test --project hosted-local   # (in frontend/) browser-only mode against :4173, no backend needed
...
- **Browser mode.** `frontend/src/local/` is a read-write port of the backend for the hosted site: SheetJS spreadsheet
  reading, header detection and fuzzy column mapping, the importer with dedupe, the offline ZIP geocoder
  (`public/geo/us_zips.txt`), the deterministic mock carrier, refresh jobs, notes, tags, export and snapshot export,
  all over an IndexedDB store (`local/db.ts`). `local/server.ts` routes the same API paths the pages already call.
